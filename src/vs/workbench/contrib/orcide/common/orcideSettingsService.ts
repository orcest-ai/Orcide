...
import { ORCIDE_SETTINGS_STORAGE_KEY } from './storageKeys.js';
import { defaultSettingsOfProvider, FeatureName, ProviderName, ModelSelectionOfFeature, SettingsOfProvider, SettingName, providerNames, ModelSelection, modelSelectionsEqual, featureNames, OrcideStatefulModelInfo, GlobalSettings, GlobalSettingName, defaultGlobalSettings, ModelSelectionOptions, OptionsOfModelSelection, ChatMode, OverridesOfModel, defaultOverridesOfModel, MCPUserStateOfName as MCPUserStateOfName, MCPUserState } from './orcideSettingsTypes.js';
...
export type OrcideSettingsState = {
...
// type RealOrcideSettings = Exclude<keyof OrcideSettingsState, '_modelOptions'>
// type EventProp<T extends RealOrcideSettings = RealOrcideSettings> = T extends 'globalSettings' ? [T, keyof OrcideSettingsState[T]] : T | 'all'


export interface IOrcideSettingsService {
	readonly _serviceBrand: undefined;
	readonly state: OrcideSettingsState; // in order to play nicely with react, you should immutably change state
	...
	dangerousSetState(newState: OrcideSettingsState): Promise<void>;
...
const _modelsWithSwappedInNewModels = (options: { existingModels: OrcideStatefulModelInfo[], models: string[], type: 'autodetected' | 'default' }) => {
	const { existingModels, models, type } = options

	const existingModelsMap: Record<string, OrcideStatefulModelInfo> = {}
...
const _stateWithMergedDefaultModels = (state: OrcideSettingsState): OrcideSettingsState => {
...
const _validatedModelState = (state: Omit<OrcideSettingsState, '_modelOptions'>): OrcideSettingsState => {
...
export const IOrcideSettingsService = createDecorator<IOrcideSettingsService>('OrcideSettingsService');
class OrcideSettingsService extends Disposable implements IOrcideSettingsService {
	_serviceBrand: undefined;

	private readonly _onDidChangeState = new Emitter<void>();
	readonly onDidChangeState: Event<void> = this._onDidChangeState.event; // this is primarily for use in react, so react can listen + update on state changes

	state: OrcideSettingsState;
	...
	dangerousSetState = async (newState: OrcideSettingsState) => {
	...
	private async _readState(): Promise<OrcideSettingsState> {
		const encryptedState = this._storageService.get(ORCIDE_SETTINGS_STORAGE_KEY, StorageScope.APPLICATION)
	...
	private async _storeState() {
		try {
			...
			this._storageService.store(ORCIDE_SETTINGS_STORAGE_KEY, encryptedState, StorageScope.APPLICATION, StorageTarget.USER);
		} catch (e) {
			console.error('[OrcideSettingsService] Failed to store state:', e);
		}
	}
	...
	setGlobalSetting: SetGlobalSettingFn = async (settingName, newVal) => {
		const newState: OrcideSettingsState = {
			...this.state,
			globalSettings: {
	...
	toggleModelHidden(providerName: ProviderName, modelName: string) {
		...
		const newModels: OrcideStatefulModelInfo[] = [
...
registerSingleton(IOrcideSettingsService, OrcideSettingsService, InstantiationType.Eager);
